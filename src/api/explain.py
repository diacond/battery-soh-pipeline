...
LG CNS AI(AX)/현대차 R&D AI/AX JD가 요구하는 "LLM 기반 Agentic 서비스"의
가장 작은 형태: 예측값(숫자)을 실무자가 바로 이해할 수 있는 자연어
진단 코멘트로 바꿔준다.

설계 원칙 - "AI를 어떻게 통제하는가"(LG에너지솔루션 JD가 명시적으로 물어보는
질문)에 대한 답이기도 하다.
  1. LLM은 있으면 좋은 것(nice-to-have)이지 필수 경로가 아니다.
     ANTHROPIC_API_KEY가 없거나 호출이 실패해도 서비스는 규칙 기반
     설명으로 정상 응답해야 한다 (fail-open, not fail-closed).
  2. 숫자 판단(임계값 비교, 경고 여부)은 LLM에 맡기지 않는다.
     LLM은 "왜 그런지 설명"만 하고, "위험하다/아니다"는 결정론적 규칙이 내린다.
     환각(hallucination)이 안전 관련 판단에 영향을 주지 않게 하기 위함.
"""
